...
(function($, window, document, undefined) {
	...
	if (s=ua.match(/opera.([\d.]+)/)) {
		gridBrowser.isOpera = true;
	}else if(browserMatch.browser=="IE"&&browserMatch.version==11){
		gridBrowser.isIE11 = true;
		gridBrowser.isIE = true;
	}else if (s=ua.match(/chrome\/([\d.]+)/)) {
		gridBrowser.isChrome = true;
		gridBrowser.isStandard = true;
	} else if (s=ua.match(/version\/([\d.]+).*safari/)) {
		gridBrowser.isSafari = true;
		gridBrowser.isStandard = true;
	} else if (s=ua.match(/gecko/)) {
		//add by licza : support XULRunner
		gridBrowser.isFF = true;
		gridBrowser.isStandard = true;
	...
	if((navigator.platform == "Mac68K") || (navigator.platform == "MacPPC") || (navigator.platform == "Macintosh") || (navigator.platform == "MacIntel")){
		//gridBrowser.isIOS = true;
		gridBrowser.isMac = true;
	}

	if((navigator.platform == "Win32") || (navigator.platform == "Windows")){
		gridBrowser.isWin = true;
	}

	if((navigator.platform == "X11") && !gridBrowser.isWin && !gridBrowser.isMac){
		gridBrowser.isUnix = true;
	}
    if((String(navigator.platform).indexOf("Linux") > -1)){
    	gridBrowser.isLinux = true;
    }
	...
	gridBrowser.version = version ? (browserMatch.version ?  browserMatch.version : 0) : 0;
	if (gridBrowser.isIE) {
		var intVersion = parseInt(gridBrowser.version);
		var mode = document.documentMode;
		if(mode == null){
			if (intVersion == 6 || intVersion == 7) {
				gridBrowser.isIE8_BEFORE = true;
			}
		}
		else{
			if(mode == 7){
				...
				gridBrowser.isSTANDARD = true;
				gridBrowser.isIE10_ABOVE = true;
			}
			else{
				gridBrowser.isSTANDARD = true;
			}
			if (intVersion == 8) {
				gridBrowser.isIE8_CORE = true;
			}
			else if (intVersion == 9) {
				gridBrowser.isIE9_CORE = true;
			}
...
//			editable: gridOptions.editable,
	...
	gridComp.prototype = {
		...
		initGridCompColumn: function() {
			var oThis = this;
			oThis.gridCompColumnArr = new Array(); 
			oThis.gridCompColumnFixedArr = new Array(); 
			oThis.basicGridCompColumnArr = new Array(); 
			oThis.gridCompLevelColumn = new Array(); 
...
//					if( Math.abs(parseInt(oThis.mouseDownX) - parseInt(oThis.mouseUpX)) <=5 && Math.abs(parseInt(oThis.mouseDownY) - parseInt(oThis.mouseUpY)) <=5){
			...
			$(document).on('click',function(e){
				...
				if(oThis.options.editable && oThis.options.editType == 'default'){
					var $e = $(e.target);
					if(!($(e.target).closest('.u-grid-content-td-div').length > 0)){
						oThis.editClose();
					}
				}
			});
